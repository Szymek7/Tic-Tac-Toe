...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for (int i=0; i<sign.length; i++) {
            //przekątne
            if (grid[0][2] == sign[i] && grid[1][1] == sign[i] && grid[2][0] == sign[i])
                return sign[i] + " wins";
            if (grid[0][0] == sign[i] && grid[1][1] == sign[i] && grid[2][2] == sign[i])
                return sign[i] + " wins";

            //wiersze lub kolumny
            for(int z=0; z<grid.length; z++) {
                if (grid[0][z] == sign[i] && grid[1][z] == sign[i] && grid[2][z] == sign[i])
                    return sign[i] + " wins";
                if (grid[z][0] == sign[i] && grid[z][1] == sign[i] && grid[z][2] == sign[i])
                    return sign[i] + " wins";
            }
        ...
        if (filledFields == grid.length * grid[0].length){
            return "Tie";
        }
